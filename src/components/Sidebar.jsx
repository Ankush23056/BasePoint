...
const Sidebar = ({ isMobileMenuOpen, setIsMobileMenuOpen }) => {
  ...
  const menuItems = [
    ...
    { title: 'Transactions', icon: <ArrowRightLeft className="w-5 h-5" />, path: '#' },
    { title: 'Insights', icon: <PieChart className="w-5 h-5" />, path: '#' },
    { title: 'Settings', icon: <Settings className="w-5 h-5" />, path: '#' },
  ];
        ...
        <div className="flex flex-col h-full w-[260px]">
          ...
          <nav className="flex-1 px-4 py-4 space-y-2">
            {menuItems.map((item, index) => (
              ...
              >
                <div className={`${index === 0 ? 'text-indigo-600 dark:text-indigo-400' : 'text-zinc-400 group-hover:text-zinc-900 dark:group-hover:text-zinc-100'}`}>
                  {item.icon}
                </div>
                {(!isCollapsed || isMobileMenuOpen) && (
